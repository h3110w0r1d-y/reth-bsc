...
| ---- | ---------| ------------------------|----------------------|
| ArchiveNode | BSC-MainNet | [reth_static_files.tar.zst](https://pub-c5400abe5bed4adbaf8cd47467747e74.r2.dev/reth_static_files.tar.zst) | 1.654Ti |
| ArchiveNode | BSC-MainNet | [reth_db.tar.zst](https://pub-c5400abe5bed4adbaf8cd47467747e74.r2.dev/reth_db.tar.zst) | 2.579Ti |
...
2. **Extract the static files**:
   ```bash
   tar --use-compress-program=zstd -xf reth_static_files.tar.zst
   # This will create/populate the 'static_files' directory
   ```

3. **Extract the database files**:
   ```bash
   tar --use-compress-program=zstd -xf reth_db.tar.zst
...
5. **Start your BSC Reth archive node**:
   ...
   **Key parameters explained:**
   ...
   - `--db.max-size=15TB`: Sets maximum database size (adjust based on your disk space)
...
- **Disk Space**: 
  ...
  - TestNet Archive: ~1T+ free space  
  - TestNet FullNode: ~5T+ free space
